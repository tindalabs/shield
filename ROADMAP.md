...
- [ ] **Broader coverage uplift — in progress.** Overall coverage now at **69% stmts / 53% branch / 75% funcs / 71% lines** (292 tests, up from 40%/28%/37%/42% with 58 tests). Highest-leverage gaps to attack next, ordered by impact-per-test:
  ...
  Suggested next slice: the two heavy strategies — `DevToolsStrategy.ts` (25%) and `ScreenshotStrategy.ts` (35%). Together they're the largest remaining coverage gap. After that, the long-tail polish (`ContextMenuStrategy` 44%, `ExtensionStrategy` 48%, `IFrameStrategy` 58%, etc.) is straightforward but lower-leverage.
