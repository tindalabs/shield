...
- [ ] **Broader coverage uplift — in progress.** Overall coverage now at **68% stmts / 52% branch / 73% funcs / 70% lines** (274 tests, up from 40%/28%/37%/42% with 58 tests). Highest-leverage gaps to attack next, ordered by impact-per-test:
  ...
  Suggested next slice: the small-files cleanup — item #8 (`dom.ts`, `orientation.ts`, `LoggingDelegate.ts` at 0%). Easy wins, low LoC. After that the major remaining gap is `DevToolsStrategy.ts` (25%) and `ScreenshotStrategy.ts` (35%) — the two strategies with the heaviest internal state.
