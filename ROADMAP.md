...
- [ ] **Broader coverage uplift — in progress.** Overall coverage now at **52% stmts / 38% branch / 51% funcs / 53% lines** (195 tests, up from 40%/28%/37%/42% with 58 tests). Highest-leverage gaps to attack next, ordered by impact-per-test:
  ...
  Suggested next slice: `protectedContentManager.ts` (11%) + `keyboardShortcutManager.ts` (11%) — item #5.
