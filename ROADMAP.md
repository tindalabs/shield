...
- [ ] **Broader coverage uplift — in progress.** Overall coverage now at **76% stmts / 61% branch / 83% funcs / 78% lines** (372 tests, up from 40%/28%/37%/42% with 58 tests). Highest-leverage gaps to attack next, ordered by impact-per-test:
  ...
  Suggested next: the **LoggableComponent consolidation bonus** (item 9 above) — fold the 7 utility classes that still build their own `SimpleLoggingService` into the base. Solid code-quality win and lifts coverage for free since the consolidated path is already tested. After that, distribution polish (README badges, PR template, dependabot) becomes the gating work for v0.1.0.
